...
function createAvatarDownload(input, user_avatar) {

    if (user_avatar) {
        var body = document.getElementsByTagName("body")[0];
        body.classList.add("user-avatar");
    }
    ...
    $(input).fileuploader({
        ...
        thumbnails: {
            box: '<div class="fileuploader-wrapper">' +
                ...
                '<div class="fileuploader-droparea" data-action="fileuploader-input">' +
                '<i class="fileuploader-icon-main"></i>' +
                '</div>' +
                '</div>' +
                '<div class="fileuploader-menu">' +
                '<button type="button" class="fileuploader-menu-open">' +
                // '<i class="fileuploader-icon-menu"></i>' +
                '</button>' +
                '<ul>' +
                '<li><a data-action="fileuploader-input"><div class="fileuploader-icon-upload"></div> ${captions.upload}</a></li>' +
                '<li><a data-action="fileuploader-edit"><div class="fileuploader-icon-edit"></div> ${captions.edit}</a></li>' +
                '</ul>' +
                '</div>',
        ...
        captions: $.extend(true, {}, $.fn.fileuploader.languages['pl'], {
            edit: 'Edytuj',
            upload: 'Zmień',
            errors: {
                filesLimit: 'Można załadować tylko 1 plik.',
            }
        })
    });
}
...
$(document).ready(function () {
    ...
    $('.multiselect_cars select').on('change', function () {
        addValMultiselect($(this), $(this).find(":selected"), 'edit-make');
    });

    $('.multiselect_services select').on('change', function () {
        addValMultiselect($(this), $(this).find(":selected"), 'edit-service');
    });

    $('.multiselect_facilities select').on('change', function () {
        addValMultiselect($(this), $(this).find(":selected"), 'edit-facility');
    });

    $(document).on('click', function (e) {
        if ($(e.target).hasClass('multiselect__value-del')) {
            delValMultiselect($(e.target).closest('.multiselect__value'));
        }
    });


    $('.multiselect__input-all').on('click', function () {
        addAllValMultiselect($(this).closest('.multiselect').find('select'));
    });

    function initMultiselect($wrapper) {
        var $select = $wrapper.find('select');
        var $active_values = $wrapper.find('.multiselect__value');
        if ($active_values.length > 0) {
            $active_values.each(function () {
                var active_value_name = $(this).find('.multiselect__value-name').text();

                $select.find('option').each(function () {
                    var option_text = $(this).text();

                    if (option_text === active_value_name) {
                        $(this).remove();
    ...
    function addValMultiselect($select, $current_option, $class_edit) {
        var $wrapper = $select.closest('.multiselect');
        var $items = $wrapper.find('.multiselect__block');
        var current_val = $current_option.val();
        var current_text = $current_option.text();
        var item_template = '<div class="multiselect__value ' + $class_edit + '" data-value="' + current_val + '"><div class="multiselect__value-name">' + current_text + '</div><div class="multiselect__value-del"></div></div>';
        $(item_template).appendTo($items);
        $current_option.remove();
        $select.selectmenu("refresh");
    }

    function delValMultiselect($value) {
        var $select = $value.closest('.multiselect').find('select');
        var current_val = $value.data('value');
        var current_text = $value.find('.multiselect__value-name').text();
        var item_template = '<option value="' + current_val + '">' + current_text + '</option>';
        $value.remove();
        $(item_template).appendTo($select);
        $select.selectmenu("refresh");
    }

    function addAllValMultiselect($select) {
        var type = $select.attr('id');
        var edit_class;
        switch (type) {
            case "Make":
                edit_class = "edit-make";
                break;
            case "CarService":
                edit_class = "edit-service";
                break;
            case "Facility":
                edit_class = "edit-facility";
                break;
            default:
                edit_class = "";
        }
        $select.find('option').each(function () {
            if ($(this).val() !== 'option-placeholder') {
                addValMultiselect($select, $(this), edit_class);
            }
        });
    }

    function getDayWorktimeTill($day) {
        var checkItem = $day.find('.workshop-profile__worktime-item')
        if (checkItem.hasClass('not-work')) {
            return null
        } else {
            var $input_to = $day.find('.time-till');
            return $input_to.val();
        }
    }

    function getDayWorktimeFrom($day) {
        var checkItem = $day.find('.workshop-profile__worktime-item')
        if (checkItem.hasClass('not-work')) {
            return null;
        } else {
            var $input_from = $day.find('.time-from');
            return $input_from.val();
        }
    }

    $('#btnUpdateWorkshopProfile').on('click', function () {
        event.preventDefault();
        $(this).parent().html(`<div class="preloader__editworkshop">
                                <div class= "preloader__spinner active" ></div >
                            </div >`);
        var form = document.getElementById('frmEditProfile');
        var token = $('input[name="__RequestVerificationToken"]', form).val();

        var mondayfrom = getDayWorktimeFrom($('#workHoursMonday'));
        var mondaytill = getDayWorktimeTill($('#workHoursMonday'));
        var tuesdayfrom = getDayWorktimeFrom($('#workHoursTuesday'));
        var tuesdaytill = getDayWorktimeTill($('#workHoursTuesday'));
        var wednesdayfrom = getDayWorktimeFrom($('#workHoursWednesday'));
        var wednesdaytill = getDayWorktimeTill($('#workHoursWednesday'));
        var thursdayfrom = getDayWorktimeFrom($('#workHoursThursday'));
        var thursdaytill = getDayWorktimeTill($('#workHoursThursday'));
        var fridayfrom = getDayWorktimeFrom($('#workHoursFriday'));
        var fridaytill = getDayWorktimeTill($('#workHoursFriday'));
        var saturdayfrom = getDayWorktimeFrom($('#workHoursSaturday'));
        var saturdaytill = getDayWorktimeTill($('#workHoursSaturday'));
        var sundayfrom = getDayWorktimeFrom($('#workHoursSunday'));
        var sundaytill = getDayWorktimeTill($('#workHoursSunday'));
        ...
        if ($('.edit-make').length > 0) {
            $('.edit-make').each(function () {
                cars.push(this.getAttribute('data-value'));
            });
        }
        if ($('.edit-service').length > 0) {
            $('.edit-service').each(function () {
                services.push(this.getAttribute('data-value'));
            });
        }
        if ($('.edit-facility').length > 0) {
            $('.edit-facility').each(function () {
                facilities.push(this.getAttribute('data-value'));
            });
        }
        var model = {
            __RequestVerificationToken: token,
            MondayFrom: mondayfrom, MondayTill: mondaytill,
            TuesdayFrom: tuesdayfrom, TuesdayTill: tuesdaytill,
            WednesdayFrom: wednesdayfrom, WednesdayTill: wednesdaytill,
            ThursdayFrom: thursdayfrom, ThursdayTill: thursdaytill,
            FridayFrom: fridayfrom, FridayTill: fridaytill,
            SaturdayFrom: saturdayfrom, SaturdayTill: saturdaytill,
            SundayFrom: sundayfrom, SundayTill: sundaytill,
            WorkshopMakes: cars,
            WorkshopServices: services,
            WorkshopFacilities: facilities,
            Description: description,
            Website: website,
            AvatarId: avatarId, 
            RemovedImages: removedImgs,
            AddedImages: addedImgs
        };

        var url = "/workshop/updateprofile";
