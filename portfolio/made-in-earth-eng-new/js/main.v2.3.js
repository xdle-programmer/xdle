...
$(document).ready(function () {
    ...
    // $('.english input[name="phone"]').mask('+1 (999) 999-9999', {placeholder: "+1 (___) ___-____"});

    let phone_input = 0;

    // $('input[name="phone"]').each(function () {
    //     let phoneMask = IMask(
    //         document.querySelectorAll('[name="phone"]')[phone_input], {
    ...
    function openPortfolio() {
        ...
        let current_scroll;
        let portfolio_width = 1400;
        let min_width = 1430;

        if (window.outerWidth < portfolio_width) {
        ...
        $close_button.on('click', function () {
            ...
            if (window.outerWidth < min_width) {
                ...
                $portfolio_wrapper.removeClass(active_class);
                $portfolio_item.removeClass(active_class);
                $portfolio_wrapper.scrollTop(0);
            }


        });

        function calcPortfolioZoom() {
            let zoom = ((window.outerWidth - 32) / 1400).toFixed(2);
            return zoom;
        }
    }
        ...
        if ($(this).hasClass('disabled') !== true) {
            ...
            if ($form.find('input[name="email"]').length > 0) {
                if ($form.find('input[name="email"]').val().length > 0) {
                    ...
                    $form.find('input[name="hidden_email"]').val($form.find('input[name="email"]').val());
...
function onYouTubeIframeAPIReady() {
    ...
    $('.youtube-player').each(function () {
        let player_id = 'youtube-player_' + counter;

        $(this).find('.youtube-player__inner').prop('id', player_id);
        youtubeDownload($(this), player_id);
        counter++;
    });

    function youtubeDownload($player, player_id) {
        var embed = $player.data('youtube');
        var player;

        var player_control;

        $(document).on('stop_youtube', function () {
            if (player_control) {
                player_control.stopVideo();
            }
        });
