...
(function ($) {
    ...
    function compareInit() {
        let $buttons = $('.compare-vendors__button');
        let $active_button = $('.compare-vendors__button.active');
        let main_price = 50000;

        $buttons.on('click', function () {
            if ($(this).hasClass('active') !== true) {
                $buttons.removeClass('active');
                $(this).addClass('active');
                $active_button = $('.compare-vendors__button.active');
                createArray();
            }
        });
        ...
        function createArray() {
            let vendor_price = parseInt($active_button.data('price'));
            let month_array = [];
            let temp_month_price = 0;
            let year_array = [];
            let temp_year_price = 0;

            for (let i = 0; i < 120; i++) {
                let economy = temp_month_price - main_price;
                if (economy < 0) {
                    economy = 0;
                }
                month_array.push(economy);
                temp_month_price += vendor_price;
            }

            for (let i = 0; i < 10; i++) {
                let economy = temp_year_price - main_price;
                if (economy < 0) {
                    economy = 0;
                }
                year_array.push(economy);
                temp_year_price += vendor_price * 12;
            }
            ...
            function createChart() {
                var chart = new Chartist.Line('.ct-chart', {
                    labels: ['1 год', '2 года', '3 года', '4 года', '5 лет', '6 лет', '7 лет', '8 лет', '9 лет', '10 лет'],
                    series: [
                        year_array,
                    ]
                }, {
            ...
            function range_slider($slider, val_min, val_max) {
                ...
                for (let i = 0; i < month_array.length; i++) {
                    if (month_array[i] > 0 && val_start === 0) {
                        val_start = i;
                    }
                }

                changeDesc(val_start, month_array[val_start]);

                $slider_item.slider({
                    min: val_min,
                    max: val_max,
                    slide: function (event, ui) {
                        val_current = ui.value;
                        changeDesc(ui.value, month_array[ui.value]);
                    },
                ...
                function changeDesc(month, economy) {
                    let $economy = $('.range-slider__desc-item--economy .range-slider__desc-val');
                    let $month = $('.range-slider__desc-item--month .range-slider__desc-val');
            ...
            range_slider($('#price_range'), 0, 119, 30);
    ...
    $('.features-switch-header-item').on('click', function () {
        if ($(this).hasClass('active') === false) {
            $('.features-switch-header-item').removeClass('active');
            $(this).addClass('active');
            $('.features-switch').removeClass('active');
            $('[data-id="' + $(this).data('id-goal') + '"]').addClass('active');
