...
// создадим элемент с прокруткой
    ...
    function game() {
        let playState = false;
        let jumpDelay = 500;
        let $counter = $('.game__counter');
        let $person = $('.game__person');
        let current_val = 0;
        let $mouse = $('.game__mouse');
        let $donut = $('.game__donut');
        let person_width = $person.width();
        let distance = viewport_width - $person.offset().left - person_width;
        ...
        $(window).keypress(function (e) {
            if (e.keyCode === 0 || e.keyCode === 32) {
                ...
                if (!playState) {
                    ...
                    $('.game__person-wrapper').addClass('game__person-wrapper--jump');
                    setTimeout(function () {
                        $('.game__person-wrapper').removeClass('game__person-wrapper--jump');
                    }, jumpDelay);
        ...
        function startGame() {
            $('.game__person').addClass('game__person--active');
            $('.game__ground').addClass('game__ground--active');
            setMouse();
            playState = true;
            ...
            let timerId = setInterval(() => {
                current_val += 3000;
                $counter.text('€ ' + numberWithSpaces(current_val));
            }, 15);
        }
        ...
        function setMouse() {
            ...
            if (awards) {
                $tempMouse = $donut.clone().appendTo('.game');
            } else {
                $tempMouse = $mouse.clone().appendTo('.game');
            }

            let shift = 0;
            let collision = false;

            let intervalMouse = setInterval(function () {
                shift -= 10;

                if (-distance > shift && -distance - person_width < shift) {
                    if (!$('.game__person-wrapper').hasClass('game__person-wrapper--jump')) {

                        if (awards) {
                            $('.game__success').addClass('game__success--active');
                            setTimeout(function () {
                                $('.game__success').removeClass('game__success--active');
                            }, 500);
                        } else {
                            $('.game__doh').addClass('game__doh--active');
                            setTimeout(function () {
                                $('.game__doh').removeClass('game__doh--active');
                            }, 500);
                        }
                        ...
                        if (!collision) {
                            if (awards) {
                                current_val = current_val + 100000;
                            } else {
                ...
                } else if (shift < -viewport_width) {
                    $tempMouse.remove();
                    clearInterval(intervalMouse);
                }

                $tempMouse.css('transform', 'translateX(' + shift + 'px)');
            }, 10);

            setTimeout(function () {
                setMouse();
            }, randomInteger(400, 2500));
        }
